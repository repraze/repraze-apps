...
function getPostColumns({handleDelete}: {handleDelete: (id: string) => void}): DataTableColumn<Post>[] {
    return [
        {
            id: "visual",
            cell: function visualCol(row) {
                return (
                    <Icons title="Post">
                        <Icon icon={faPen} color={Colors.Primary} fixedWidth />
                        {!row.public && <Icon icon={faEyeSlash} fixedWidth transform={{size: 12, x: 8, y: 8}} />}
                        {row.featured && <Icon icon={faStar} fixedWidth transform={{size: 12, x: -8, y: -8}} />}
                        {row.listed && <Icon icon={faBars} fixedWidth transform={{size: 12, x: 8, y: -8}} />}
                    </Icons>
                );
            },
        ...
        },
        {
            ...
            cell: function titleCol(row) {
                return (
                    <Link href={`/admin/posts/${row.id}`} title={row.id || ""} style={{minWidth: 0}}>
        ...
        {
            id: "published_date",
            cell: (row) => (row.publish_date ? dateTimestampFormat(row.publish_date) : ""),
            rawCell: "publish_date",
            width: 175,
            grow: 0,
            align: "right",
        },
        {
            ...
            cell: function creationUserCol(row) {
                return (
                    row.author_users &&
                    row.author_users.map((user, index) => (
                        <Fragment key={user.username}>
                            <UserProfile user={user} href={`/admin/users/${user.username}`} />
                            {index !== 0 ? ", " : ""}
                        </Fragment>
                    ))
                );
...
export default function PostsView() {
    ...
    const [published, setPublished] = useURLParamBoolean("published");
    const [publicFlag, setPublic] = useURLParamBoolean("public");
    const [listed, setListed] = useURLParamBoolean("listed");
    const [featured, setFeatured] = useURLParamBoolean("featured");
    ...
    const views = useMemo(() => {
        const columns = getPostColumns({handleDelete});
    ...
    return (
        <AdminLayout>
            <Head>
                <title>{titleFormat("Posts")}</title>
            </Head>
            <Hero className="posts-title flex-none">
                <Wrapper width={Widths.Full}>
                    <Fields inline wrapped>
                        ...
                        <Field>
                            <Control>
                                <Tag size={Sizes.Large}>{data?.count} posts</Tag>
                            </Control>
                        </Field>
                        ...
                        <Field>
                            <Control>
                                <Button as={Link} color={Colors.Success} href={`/admin/posts/new`}>
            ...
            <Section className="posts-tools flex-none">
                <Wrapper width={Widths.Full}>
                    <Fields inline>
                        <Field expanded>
                                        ...
                                        <>
                                            ...
                                            <DropdownItem>
                                                <Select
                                                    ...
                                                    value={published === undefined ? "all" : published ? "1" : "0"}
                                                    size={Sizes.Small}
                                                    onChange={(e) => {
                                                        const value = e.target.value;
                                                        setPublic(undefined); // published sets public
                                                        setPublished(
                                            ...
                                            <DropdownItem>
                                                <Select
                                                    ...
                                                    value={featured === undefined ? "all" : featured ? "1" : "0"}
                                                    size={Sizes.Small}
                                                    onChange={(e) => {
                                                        const value = e.target.value;
                                                        setFeatured(
                                            ...
                                            <DropdownItem>
                                                <Select
                                                    ...
                                                    value={listed === undefined ? "all" : listed ? "1" : "0"}
                                                    size={Sizes.Small}
                                                    onChange={(e) => {
                                                        const value = e.target.value;
                                                        setListed(
                                            ...
                                            <DropdownItem>
                                                <Select
                                                    ...
                                                    onChange={(e) => {
                                                        const value = e.target.value;
                                                        setPublished(undefined); // published sets public
                                                        setPublic(
                                                            value === "all" ? undefined : value === "1" ? true : false
                                                        );
                                                    }}
            ...
            <Section className="posts-listing flex-1 flex">
            ...
            <Section className="posts-pagination flex-none">
                <Wrapper width={Widths.Full}>
                    <Pagination
                        pageSelected={page}
                        pageRange={1}
